...
[CustomPropertyDrawer(typeof(PrefabsDictionary))]
public class MyDictionaryDrawer1 : DictionaryDrawer<string, UnityEngine.Object> {
    ...
    {
        PrefabsDictionary newDict = new PrefabsDictionary();
        foreach (var key in _Dictionary)
        {
            if (key.Value != null)
            {
                if (!newDict.ContainsKey(key.Value.name))
                {
                    newDict.Add(new KeyValuePair<string, UnityEngine.Object>(key.Value.name, key.Value));
                }
                else
                {
                    newDict.Add(new KeyValuePair<string, UnityEngine.Object>(key.Key, key.Value));
                }
            }
            else
            {
